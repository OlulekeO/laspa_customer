...
import 'add_hour.dart';

class checkout_preview extends StatefulWidget {
  const checkout_preview({Key? key}) : super(key: key);

  @override
  _checkout_previewState createState() => _checkout_previewState();
}

class _checkout_previewState extends State<checkout_preview> {
  ...
  notifyCustomer() async {
    ...
    if (request == '1'){
      Navigator.pushReplacement(
          context, MaterialPageRoute(builder: (context) => add_hour()));

    }else{

     // print("checkout");

      String url = "https://app.prananet.io/LaspaApp/Api/Customer_Checkout2";
      final response = await http.post(url, body: {
      ...
      }else if(message.contains("Customer Already Checkedout")){

        setState(() {
        });
        String message = "You've already checked out";
        showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
            ...
            child: Container(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text("Parking Preview",style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                    const SizedBox(
                      height: 30,
                    ),
                    ...
                    const Text("You have exceeded your parking duration kindly Top up or Checkout",style: TextStyle(fontSize: 14,),),
                    const SizedBox(
                      height: 20,
                    ),
                    ...
                    //     child: Text("Customer Alert",style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold),)),
